...
### 7. 단어장 (Vocabulary)
- 플래시카드 암기 모드: 영단어 → 뜻 → 유사어(+유사어 뜻) 단계적 공개
- 랜덤 순서로 출제
- Quizlet 스타일 암기 완료 추적: 완료 표시된 단어는 "미암기 단어" 학습 시 제외
- "전체 시작"으로 모든 단어 다시 학습 가능
- 카테고리별 분류 (예: 1주차, 2주차)
- 단어 수동 추가/수정/삭제
- 이미지 OCR 업로드 (여러 장 동시 선택 가능, pytesseract 사용)
- **유사어 뜻 자동 검색**: Free Dictionary API (https://dictionaryapi.dev) 사용
  - 단어 저장 시 유사어의 영어 뜻을 자동으로 검색하여 저장
  - 무료, API 키 불필요
  - 영영사전 기반 (영어 정의 제공)

### 8. 비밀번호 찾기
- 로그인 페이지에서 "비밀번호 찾기" 탭
- 아이디 입력 시 비밀번호 표시 (회원가입 시 평문 저장)
